...
#if defined(NDEBUG)
# error "MiTronCoinX cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2c101ba909dac393cb592f889dadc70a37c3526cf8e22ce5c882883e97a3ab70");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // MiTronCoinX: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "MiTronCoinX Signed Message:\n";
...
{
    // MiTronCoinX: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // MiTronCoinX: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // MiTronCoinX: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // MiTronCoinX: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // MiTronCoinX: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // MiTronCoinX: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7759f65221fdbde5e276b3215bddbd16edbf8431c663f03bc16a99872c544a6b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Doug Jones defeats Roy Moore Alabama aenate election";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1535347695;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084934761;

        if (fTestNet)
        {
            block.nTime    = 1535347658;
            block.nNonce   = 386186603;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xe08f06cdffe9fb9c223f99bbe11a6fd8bb74f65f3c73cf756790d61eedd3f5ab"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // MiTronCoinX: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MiTronCoinXMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MiTronCoinXMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MiTronCoinXMiner(CWallet *pwallet)
{
    printf("MiTronCoinXMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mitroncoinx-miner");
    ...
    try { loop {
        ...
        printf("Running MiTronCoinXMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MiTronCoinXMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MiTronCoinXMiner, pwallet));
}
